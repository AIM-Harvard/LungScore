...
def lung_vol_pix(seg_lung):
    ...
    Returns:
        lung volume --> represented by counting the pixels values that are not representing air
    """
...
def scans_w_outofrange_values(seg_lung):
    ...
    Args:
        seg_lung (numpy_array): numpy array of segmented lung
    Returns: Print statment with corrupted max and min values or no corrupted values 
...
def clipping_values(seg_lung_tensor):
    ...
    Returns:
        seg_lung_tensor_clipped --> segmented lung tensor with values clipped to 3071 and -1024
    """
