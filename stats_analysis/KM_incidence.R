"""
Deep-learning biomarker for Lung Health stats_analysis - KM incidence analysis
...
############################
# KM Incidence Plot function
#############################
plot_KM_unzoomed_fivesplit_incidence <- function(df, time_col, event_col, group_col, title, xlab, ylab, pdf_file, cohort_name, xlim, ylim = c(0, 1), offset_annotation = 0.1, offset_annotation_distance = 5, YEAR = 12) {
...
# Fit survival curves
#fit_surv <- survfit(surv_obj ~ group_rev, data = df_for_plot)
...
# for 4 risk groups
ci_group1 <- c(ci[1], ci[5]) 
ci_group2 <- c(ci[2], ci[6])
ci_group3 <- c(ci[3], ci[7])
ci_group4 <- c(ci[4], ci[8])
...
p_text_group4 <- ifelse(pval[1] < 0.001, "<0.001", sprintf("=%.3f", pval[4]))
...
# Transforming survival probability into cumulative incidence
fit_transformed <- fit
fit_transformed$surv <- 1 - fit_transformed$surv
...
g$plot <- g$plot + 
  ...
  # very high
  annotate("text", x = 0.1, y = (offset_annotation + 0.025 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = paste("very high:"), hjust = "left", size = 3.5) + 
  annotate("text", x = 2, y = (offset_annotation + 0.025 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = paste("Ref"), hjust = "left", size = 3.5)

  # high group
  annotate("text", x = 0.1, y = (offset_annotation + 0.02 /offset_annotation_distance  * (1-ylim[[1]]) + ylim[[1]]),  label = paste("high:"), hjust = "left", size = 3.5) +
  annotate("text", x = 2, y = (offset_annotation + 0.02 /offset_annotation_distance  * (1-ylim[[1]]) + ylim[[1]]), label = hr_text_group1, hjust = "left", size = 3.5) +
  
  # moderate group
  annotate("text", x = 0.1, y = (offset_annotation + 0.015 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = paste("moderate:"), hjust = "left", size = 3.5) +
  annotate("text", x = 2, y = (offset_annotation + 0.015 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = hr_text_group2, hjust = "left", size = 3.5) +
  
  # low group
  annotate("text", x = 0.1, y = (offset_annotation + 0.01  /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = paste("low:"), hjust = "left", size = 3.5) +
  annotate("text", x = 2, y = (offset_annotation + 0.01  /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = hr_text_group3, hjust = "left", size = 3.5) +
  
  # very low group
  annotate("text", x = 0.1, y = (offset_annotation + 0.005 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = paste("very low:"), hjust = "left", size = 3.5) +
  annotate("text", x = 2, y =  (offset_annotation + 0.005 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = hr_text_group4, hjust = "left", size = 3.5) +
  
  # 4 df p-value
  annotate("text", x = 5.2, y = (offset_annotation + 0.005 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = p_value_catdf_reduced, hjust = "left", size = 3.5) +
  
  
  # no of patients and HZ 95 ci 
  annotate("text", x = 0.1, y = (offset_annotation + 0.035 /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]), label = paste("n=", total_patients), hjust = "left", size = 3.5) + 
  annotate("text", x = 2, y = (offset_annotation + 0.03  /offset_annotation_distance * (1-ylim[[1]]) + ylim[[1]]),label = paste("HR (95% CI)"), hjust = "left", size = 3.5) +
...
# Example usage for lung cancer death in NLST
# read data
df_NLST <- read.csv('/mnt/data/NLST_test.csv')

# Lung cancer death NLST - Figure 3b
plot_KM_unzoomed_fivesplit_incidence(
df = df_NLST, 
time_col = "ICD_OSYEARS_12y",   
event_col = "LUNGCANCER_DEATH_12y", 
group_col = "lunghealth_cat", # 5 risk groups
title = "Lung cancer death in", 
xlab = "Years since randomization", 
ylab = "Cumulative lung cancer mortality", 
pdf_file = "mnt/outcome_analysis/Figures_LH/LC_DEATH_KM_NLST.pdf", 
cohort_name = "NLST",
xlim = c(0, 12), 
ylim = c(0, 0.15),
offset_annotation = 0.1,
offset_annotation_distance = 0.8,
YEAR = 12
)
